...
import RequestFlfwJigouData from './storage/RequestFlfwJigouData.js';
import RequestFlfwRenyuanData from './storage/RequestFlfwRenyuanData.js';
...
var Flfw = React.createClass({
	getInitialState:function(){
		...
		return{
			...
			shijianbianliang:0,
			datasum:RequestFlfwJigouData.flfwarr,
			newstateflfw:0
		}
	},
	onUpdate:function(newState){
		...
		if(this.state.yanzhengflfwright=="jigou"){
			...
			this.setState({
				datasum:RequestFlfwRenyuanData.flfwbrr,
				newstateflfw:newState
			});
		}
	},
	...
	render(){
		return (		
			<div className="Flfwpage">
				<Headerbar index="法律服务" prev={[{link:'/',icon:'chevron-left'}]} />
